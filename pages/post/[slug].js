              ...
              <div className='flex gap-2 justify-center flex-wrap'>
                {post.categories.map((category) => (
                  <Link key={category.slug} href={`/category/${category.slug}`}>
                    <div className='px-2 py-1 text-xs sm:text-sm xl:text-base rounded bg-gray-200 cursor-pointer'>{category.name}</div>
                  </Link>
                ))}
              </div>
              <p className='text-center text-base font-light text-gray-600'>
                Published <DateFormater dateString={post.createdAt} /> by {post.author.name}
              </p>
